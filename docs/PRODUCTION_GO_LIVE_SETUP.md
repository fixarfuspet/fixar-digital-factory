...
1. `.env.production` yalnız sunucuda, `0600` izinle oluşturulur; example dosyalardaki placeholder'lar güçlü secretlarla değiştirilir. `FIXAR_DEV_TEST_PASSWORD` production'a konmaz.
...
4. Migration öncesi doğrulanmış backup zorunludur. `deploy-production.sh` varsayılan dry-run'dır; `--execute`, `FIXAR_VERIFIED_BACKUP` ve incelenmiş `FIXAR_MIGRATION_SQL` olmadan yazma yapmaz.
