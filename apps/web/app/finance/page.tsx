...
export default function FinanceDashboard() {
  ...
  useEffect(() => {
    void (async () => {
      const result = await apiRequest<Dashboard>(`/api/backend/api/v1/financial-transactions/dashboard?reportingCurrency=${currency}`);
  ...
  return <main className="min-h-screen bg-zinc-950 p-4 text-white md:p-6"><div className="mx-auto max-w-7xl space-y-5">
    ...
    <section className="flex flex-wrap gap-2">{[["cash-transactions", "Kasa Hareketleri"], ["income-expense", "Gelir / Gider"]].flatMap(([report, label]) => [<a className={control} key={`${report}-xlsx`} href={`/api/backend/api/v1/finance-exports/${report}.xlsx?currency=${currency}`}>{label} XLSX</a>, <a className={control} key={`${report}-pdf`} href={`/api/backend/api/v1/finance-exports/${report}.pdf?currency=${currency}`}>{label} PDF</a>])}</section>
