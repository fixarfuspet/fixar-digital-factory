...
export function fmt(v: any) { if (v === null || v === undefined || v === "") return "—"; if (typeof v === "string" && /^\d{4}-\d{2}-\d{2}T/.test(v)) return new Date(v).toLocaleString("tr-TR"); return String(v); }
export function label(k: string) { return ({ requestNumber: "Talep No", maintenanceWorkOrderNumber: "İş Emri No", planCode: "Plan Kodu", assetCode: "Varlık Kodu", assetName: "Varlık", asset: "Ekipman", title: "Başlık", name: "Ad", priority: "Öncelik", status: "Durum", reportedAt: "Bildirim", plannedStart: "Planlanan", nextDueDate: "Sonraki Bakım", workType: "İş Türü", frequencyType: "Sıklık", assetType: "Tür", criticality: "Kritiklik", maintenanceStrategy: "Strateji", isActive: "Aktif" } as Row)[k] ?? k.replace(/([A-Z])/g, " $1"); }
