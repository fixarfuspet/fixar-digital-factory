...
export function SupplierFinanceView({title,endpoint}:{title:string;endpoint:string}){
 ...
 useEffect(()=>{void(async()=>{const r=await apiRequest<Record<string,unknown>[]>(`${API_PROXY}/${endpoint}`);if(r.status===403){setMessage("Bu işlem için tedarikçi finans yetkiniz bulunmuyor.");return}if(!r.ok){setMessage("Veriler alınamadı.");return}setRows(r.data??[]);setMessage("")})()},[endpoint]);
