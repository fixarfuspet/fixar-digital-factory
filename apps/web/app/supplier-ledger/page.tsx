...
export default function Page() {
  ...
  useEffect(() => { void (async () => setSuppliers((await apiRequest<Supplier[]>("/api/backend/api/v1/suppliers")).data ?? []))(); }, []);
  async function load(id: string, unit = currency) { if (!id) return; const result = await apiRequest<Entry[]>(`/api/backend/api/v1/supplier-ledger/supplier/${id}/statement`); setEntries((result.data ?? []).filter(x => x.currency === unit)); setMessage(result.ok ? "" : "Tedarikçi cari bilgileri yüklenemedi."); }
