...
public class CustomersController(ApplicationDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> Summary(Guid id, CancellationToken ct)
    {
        ...
        var receivables=db.CustomerReceivables.AsNoTracking().Where(x=>x.CustomerId==id&&!x.IsCancelled);var collections=db.CustomerCollections.AsNoTracking().Where(x=>x.CustomerId==id&&!x.IsReversed&&x.Status!="Draft"&&x.Status!="Cancelled");
        var result = new { TotalOrders = await orders.CountAsync(ct), ActiveOrders = await orders.CountAsync(x => x.IsActive && !x.IsCancelled && x.Status != "Completed", ct), TotalOrderedPairs = await orders.SelectMany(x => x.Items).SumAsync(x => (int?)x.QuantityPairs, ct) ?? 0, TotalProducedPairs = await orders.SelectMany(x => x.Items).SumAsync(x => (int?)x.ProducedPairs, ct) ?? 0, TotalShippedPairs = await orders.SelectMany(x => x.Items).SumAsync(x => (int?)x.ShippedPairs, ct) ?? 0, OpenOrderAmountByCurrency = await orders.Where(x => x.IsActive && !x.IsCancelled && x.Status != "Completed").GroupBy(x => x.Currency).Select(x => new { Currency = x.Key, Amount = x.Sum(o => o.GrandTotal) }).ToListAsync(ct),ReceivableByCurrency=await receivables.GroupBy(x=>x.Currency).Select(g=>new{Currency=g.Key,Amount=g.Sum(x=>x.OriginalAmount)}).ToListAsync(ct),CollectedByCurrency=await collections.GroupBy(x=>x.Currency).Select(g=>new{Currency=g.Key,Amount=g.Sum(x=>x.Amount)}).ToListAsync(ct),OutstandingByCurrency=await receivables.GroupBy(x=>x.Currency).Select(g=>new{Currency=g.Key,Amount=g.Sum(x=>x.OutstandingAmount)}).ToListAsync(ct),OverdueByCurrency=await receivables.Where(x=>x.OutstandingAmount>0&&x.DueDate<DateTime.UtcNow.Date).GroupBy(x=>x.Currency).Select(g=>new{Currency=g.Key,Amount=g.Sum(x=>x.OutstandingAmount)}).ToListAsync(ct),OpenReceivableCount=await receivables.CountAsync(x=>x.OutstandingAmount>0,ct),OverdueReceivableCount=await receivables.CountAsync(x=>x.OutstandingAmount>0&&x.DueDate<DateTime.UtcNow.Date,ct),LastCollectionDate=await collections.MaxAsync(x=>(DateTime?)x.CollectionDate,ct),UnallocatedCollectionByCurrency=await collections.GroupBy(x=>x.Currency).Select(g=>new{Currency=g.Key,Amount=g.Sum(x=>x.UnallocatedAmount)}).ToListAsync(ct), LastOrderDate = await orders.MaxAsync(x => (DateTime?)x.OrderDate, ct) };
