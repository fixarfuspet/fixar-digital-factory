...
public class OrdersController(ApplicationDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> List(Guid? customerId, Guid? productId, string? status, DateTime? dateFrom, DateTime? dateTo, DateTime? dueFrom, DateTime? dueTo, string? currency, string? search, bool? isActive, CancellationToken ct)
    {
        ...
        var rows = await q.OrderByDescending(x => x.OrderDate).Select(x => new { x.Id, x.OrderNumber, x.OrderDate, x.CustomerId, x.Customer.CustomerCode, CustomerName = x.Customer.CompanyName ?? x.Customer.Name, x.Currency, x.ExpectedPaymentMethod, x.PaymentTermDays, x.DueDate, ItemCount = x.Items.Count(i => i.IsActive && !i.IsCancelled), TotalPairs = x.Items.Where(i => i.IsActive && !i.IsCancelled).Sum(i => i.QuantityPairs), ProducedPairs = x.Items.Sum(i => i.ProducedPairs), CutPairs = x.Items.Sum(i => i.CutPairs), ShippedPairs = x.Items.Sum(i => i.ShippedPairs), x.Subtotal, x.DiscountAmount, x.TaxAmount, x.GrandTotal, x.Status, x.IsActive, CreatedAt = x.Created, UpdatedAt = x.LastModified }).ToListAsync(ct);
    ...
    private async Task<string?> Validate(OrderRequest r, Order? existing, CancellationToken ct)
    {
        ...
        foreach (var i in r.Items)
        {
            ...
            if (existing is not null && i.Id.HasValue && i.Id != Guid.Empty)
            {
                ...
                if (i.QuantityPairs < old.ProducedPairs) return "Miktar üretilen çift sayısının altına indirilemez.";
