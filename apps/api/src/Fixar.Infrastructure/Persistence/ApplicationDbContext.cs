...
public DbSet<MaintenanceAsset> MaintenanceAssets=>Set<MaintenanceAsset>(); public DbSet<MaintenanceRequest> MaintenanceRequests=>Set<MaintenanceRequest>(); public DbSet<MaintenanceWorkOrder> MaintenanceWorkOrders=>Set<MaintenanceWorkOrder>(); public DbSet<PreventiveMaintenancePlan> PreventiveMaintenancePlans=>Set<PreventiveMaintenancePlan>(); public DbSet<MaintenanceChecklistTemplate> MaintenanceChecklistTemplates=>Set<MaintenanceChecklistTemplate>(); public DbSet<MaintenanceChecklistTemplateItem> MaintenanceChecklistTemplateItems=>Set<MaintenanceChecklistTemplateItem>(); public DbSet<MaintenanceChecklistResult> MaintenanceChecklistResults=>Set<MaintenanceChecklistResult>(); public DbSet<MaintenancePartUsage> MaintenancePartUsages=>Set<MaintenancePartUsage>();
    ...
    protected override void OnModelCreating(ModelBuilder builder)
    {
        ...
        builder.Entity<FinancialTransaction>().HasIndex(x=>new{x.FinanceCategoryId,x.TransactionDate});
        builder.Entity<FinancialTransaction>().HasIndex(x=>new{x.SupplierId,x.TransactionDate});
        builder.Entity<FinancialTransaction>().HasIndex(x=>new{x.CustomerId,x.TransactionDate});
        builder.Entity<FinancialTransaction>().HasIndex(x=>x.BusinessReference).IsUnique().HasFilter("\"BusinessReference\" IS NOT NULL AND \"IsReversed\" = FALSE");
        ...
        builder.Entity<FinancialTransaction>().HasOne(x=>x.FinanceCategory).WithMany().HasForeignKey(x=>x.FinanceCategoryId).OnDelete(DeleteBehavior.Restrict);
        builder.Entity<FinancialTransaction>().HasOne(x=>x.Supplier).WithMany().HasForeignKey(x=>x.SupplierId).OnDelete(DeleteBehavior.Restrict);
        builder.Entity<FinancialTransaction>().HasOne(x=>x.PurchaseOrder).WithMany().HasForeignKey(x=>x.PurchaseOrderId).OnDelete(DeleteBehavior.Restrict);
        builder.Entity<FinancialTransaction>().HasOne(x=>x.Order).WithMany().HasForeignKey(x=>x.OrderId).OnDelete(DeleteBehavior.Restrict);
        builder.Entity<FinanceCategory>().HasIndex(x=>x.Code).IsUnique();
        builder.Entity<FinanceCategory>().HasIndex(x=>new{x.CategoryType,x.IsActive});
        builder.Entity<FinanceCategory>().HasOne(x=>x.ParentCategory).WithMany(x=>x.Children).HasForeignKey(x=>x.ParentCategoryId).OnDelete(DeleteBehavior.Restrict);
        builder.Entity<AccountReconciliation>().HasIndex(x=>x.ReconciliationNumber).IsUnique();
        builder.Entity<AccountReconciliation>().HasIndex(x=>new{x.AccountPartyType,x.CustomerId,x.SupplierId,x.Currency,x.PeriodEnd});
        builder.Entity<AccountReconciliation>().HasOne(x=>x.Customer).WithMany().HasForeignKey(x=>x.CustomerId).OnDelete(DeleteBehavior.Restrict);
        builder.Entity<AccountReconciliation>().HasOne(x=>x.Supplier).WithMany().HasForeignKey(x=>x.SupplierId).OnDelete(DeleteBehavior.Restrict);
